...
int _start(int argc, char **argv) {
    if (argc < 3) { my_write(1, "Usage: mv <src> <dst>\n"); sys_exit(1); }

    /* Try rename first */
    if (sys_rename(argv[1], argv[2]) == 0) sys_exit(0);

    /* Fall back to cp + unlink */
    int64_t src_fd = sys_open(argv[1], 0);
    if (src_fd < 0) { my_write(1, "mv: cannot open source\n"); sys_exit(1); }
    int64_t dst_fd = sys_open(argv[2], O_CREAT | 2);
    if (dst_fd < 0) { sys_close((int)src_fd); sys_exit(1); }
