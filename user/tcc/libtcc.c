...
#include "tccgen.c"
#include "tccelf.c"
/* tccrun.c (the -run/JIT mode) is not vendored — see tcc.h's TCC_IS_NATIVE
 * override above. Its 4 exported symbols (dlopen/dlclose/dlerror/dlsym,
 * only used for -run's live symbol resolution) are provided instead by
 * user/tcc/hobbyos_compat.c as always-fail stubs. */
