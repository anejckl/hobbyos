...
static void parse_number(const char *p)
{
    ...
    *q++ = t;
    b = 10;
    if (t == '.') {
        tcc_error("floating point constants are not supported on HobbyOS");
    } else if (t == '0') {
        if (ch == 'x' || ch == 'X') {
            q--;
            ch = *p++;
    ...
    if (ch == '.' ||
        ((ch == 'e' || ch == 'E') && b == 10) ||
        ((ch == 'p' || ch == 'P') && (b == 16 || b == 2))) {
        /* Floating point constants are not supported on HobbyOS: this
         * kernel's -mno-sse (mandatory — ISRs never save SSE/FPU state,
         * so any process using it would have its float registers silently
         * corrupted across a preemption) means the compiler itself cannot
         * be built with real double/float arithmetic for this target, so
         * TCC cannot support float source-language constants either. */
        tcc_error("floating point constants are not supported on HobbyOS");
    } else {
