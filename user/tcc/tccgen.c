...
/* Floating point is not supported on HobbyOS (see gen_opif below) — this
 * kernel's -mno-sse means no code here can use real double arithmetic, so
 * ieee_finite (float NaN/Inf detection) has no legitimate caller left. */
...
/* Floating point is not supported on HobbyOS: this kernel's -mno-sse
 * (mandatory — ISRs never save SSE/FPU state, so any process using it
 * would have its float registers silently corrupted across a preemption)
 * means no real double/float arithmetic can run here. This function is
 * unreachable by construction (parse_btype rejects float/double/long
 * double as a type — see tccgen.c's type parser), kept only so the
 * dispatch table in gen_op() still has something valid to call. */
static void gen_opif(int op)
{
    (void)op;
    tcc_error("floating point operations are not supported on HobbyOS");
}
