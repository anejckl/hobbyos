 ...
 *
 * Includes the kernel's string.c directly so we test the actual
 * kernel implementation, not libc.
 */

/* Block kernel headers FIRST — before any system includes */
#define COMMON_H
#define STRING_H
 ...
 * string.c includes "string.h" which includes "common.h" — both blocked
 * by guards above. Types come from stubs.h (<stdint.h>, <stddef.h>). */
